...
async function loadProducts(initialLoad = true) {
    ...
    try {
        ...
        // Lazy Loading: تحميل 12 منتج فقط في البداية
        let q;
        if (initialLoad && !window.allProductsLoaded) {
            q = window.firebaseModules.query(
                productsRef, 
                window.firebaseModules.orderBy("createdAt", "desc"),
                window.firebaseModules.limit(12)
            );
            console.log('🚀 [Lazy Loading] تحميل 12 منتج أولي فقط');
        } else {
            q = window.firebaseModules.query(
                productsRef, 
                window.firebaseModules.orderBy("createdAt", "desc")
            ...
            console.log('📦 تحميل جميع المنتجات');
        }
        ...
        if (initialLoad && allProducts.length === 0) {
        ...
        console.log(`✅ تم تحميل ${newProducts.length} منتج - الإجمالي: ${allProducts.length}`);
...
function setupInfiniteScroll() {
    ...
    window.addEventListener('scroll', async () => {
        // التحقق من الوصول لنهاية الصفحة
        if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight - 800) {
            
            // إذا كان هناك منتجات محملة لم تعرض بعد
            if (displayedProductsCount < allProducts.length) {
                displayedProductsCount += productsPerPage;
                displayProducts(allProducts);
                console.log(`📊 [Infinite Scroll] عرض ${displayedProductsCount} من ${allProducts.length}`);
            }
            // إذا عرضنا كل المنتجات ولم نحمل كل المنتجات من Firebase
            else if (!window.allProductsLoaded && !isLoadingMore) {
                isLoadingMore = true;
                console.log('🔄 [Infinite Scroll] تحميل المزيد من المنتجات...');
    ...
    console.log('✅ [Infinite Scroll] تم تفعيل التحميل التلقائي');
}
