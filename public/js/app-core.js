// app-core.js - الدوال الأساسية والتهيئة (الإصدار المحسن والمصحح)
...
window.favorites = [];
window.allProducts = [];
window.allOrdersArray = [];
window.siteCurrency = 'SDG ';
window.siteSettings = {};
window.selectedProductForQuantity = null;
window.directPurchaseItem = null;
window.navigationHistory = ['home'];
...
function showToast(message, type = 'info', duration = 3000) {
    if (typeof window.showToast === 'function') {
        ...
        toast.style.cssText = `
            ...
            font-family: 'Cairo'; animation: slideIn 0.3s ease;
...
function isValidEmail(email) {
    if (typeof window.validateEmail === 'function') return window.validateEmail(email);
    return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
}

function isValidPhone(phone) {
    if (typeof window.isValidPhone === 'function') return window.isValidPhone(phone);
...
function formatSudanPhone(phone) {
    if (typeof window.formatSudanPhone === 'function') return window.formatSudanPhone(phone);
    if (!phone) return '';
...
const SecurityManager = {
    validateSession: function() {
        ...
        try {
            ...
            if (data.isAdmin && !window.firebaseAuth?.currentUser) {
    ...
    forceLogout: function() {
        ...
        if (window.firebaseAuth) window.firebaseModules?.signOut(window.firebaseAuth);
...
async function initializeFirebaseApp(appName = 'DefaultApp') {
    if (window.firebaseApp && appName === 'DefaultApp') {
        return { 
    ...
    try {
        ...
        if (typeof window.initializeFirebaseUnified === 'function') {
            ...
            if (instance) {
                window.firebaseApp = instance.app;
                window.firebaseAuth = instance.auth;
                window.firebaseDb = instance.db;
                window.firebaseStorage = instance.storage;
                
                // تعيين المراجع المختصرة أيضاً للتوافق
        ...
        if (appName === 'DefaultApp') {
            ...
            // تعيين المراجع المختصرة للتوافق
            window.db = db;
            window.auth = auth;
        ...
        if (loader) {
            loader.innerHTML = `
                <div style="text-align: center; padding: 30px;">
                    ...
                    <p style="color: var(--gray-color); margin-bottom: 20px;">تعذر تحميل المكتبات المطلوبة.</p>
    ...
    try {
        return await loadWithCache('siteConfig', async () => {
            ...
            if (configSnap.exists()) {
                ...
                window.siteCurrency = settings.currency || 'SDG ';
...
function updateUIWithSettings() {
    ...
    if (window.siteSettings.storeName) {
        safeElementUpdate('dynamicTitle', window.siteSettings.storeName + ' - متجر العطور ومستحضرات التجميل');
        safeElementUpdate('siteStoreName', window.siteSettings.storeName);
        safeElementUpdate('footerStoreName', window.siteSettings.storeName);
    }
    ...
    for (const [elementId, settingKey] of Object.entries(footerElements)) {
        if (window.siteSettings[settingKey]) safeElementUpdate(elementId, window.siteSettings[settingKey]);
    }
    
    const aboutStoreDesc = document.getElementById('aboutStoreDescription');
    if (aboutStoreDesc && window.siteSettings.aboutUs) aboutStoreDesc.innerHTML = window.siteSettings.aboutUs;
    
    const whatsappLink = document.getElementById('contactWhatsapp');
    if (whatsappLink) {
        const whatsappPhone = window.siteSettings.phone ? window.siteSettings.phone.replace(/\D/g, '') : '249933002015';
        whatsappLink.href = `https://wa.me/${whatsappPhone}`;
    }

    const instagramLink = document.getElementById('contactInstagram');
    if (instagramLink && window.siteSettings.instagramUrl) instagramLink.href = window.siteSettings.instagramUrl;

    const emailText = document.getElementById('contactEmailText');
    const emailLink = document.getElementById('contactEmail');
    if (emailText && window.siteSettings.email) {
        emailText.textContent = window.siteSettings.email;
        if (emailLink) emailLink.href = `mailto:${window.siteSettings.email}`;
    }

    const phoneText = document.getElementById('contactPhoneText');
    const phoneLink = document.getElementById('contactPhone');
    if (phoneText && window.siteSettings.phone) {
        phoneText.textContent = window.siteSettings.phone;
        if (phoneLink) phoneLink.href = `tel:${window.siteSettings.phone}`;
    }

    const addressText = document.getElementById('contactAddressText');
    if (addressText && window.siteSettings.address) addressText.textContent = window.siteSettings.address;
    ...
    for (const [elementId, settingKey] of Object.entries(socialLinks)) {
        ...
        if (element) {
            element.href = window.siteSettings[settingKey] || '#';
            element.style.display = window.siteSettings[settingKey] ? 'flex' : 'none';
        }
    }

    if (window.siteSettings.logoUrl) {
        const logoElements = [
        ...
        logoElements.forEach(el => {
            if (el) el.src = optimizeImageUrl(window.siteSettings.logoUrl, 100);
        });
    }
}
...
function filterMainProducts(filterType, btn) {
    ...
    let filtered;
    if (filterType === 'all') filtered = window.allProducts;
    else filtered = window.allProducts.filter(p => p[filterType] === true || p[filterType] === 'true');
...
async function syncUserDataFromFirestore() {
    ...
    try {
        ...
        if (userSnap.exists()) {
            ...
            if (data.cart && Array.isArray(data.cart)) window.cartItems = data.cart;
            if (data.favorites && Array.isArray(data.favorites)) window.favorites = data.favorites;
...
async function toggleFavorite(productId) {
    ...
    const index = window.favorites.findIndex(f => f.id === productId);
    if (index === -1) {
...
function updateFavoritesDisplay() {
    ...
    favoritesList.innerHTML = window.favorites.map(product => {
            ...
            <div class="product-card" data-id="${product.id}">
                ...
                <div class="product-info">
                    ...
                    <div class="product-price">
                        <span class="current-price">${formatNumber(product.price)} ${window.siteCurrency}</span>
                    </div>
...
function cleanupUnusedData() {
    if (window.allProducts.length > 100) window.allProducts = window.allProducts.slice(0, 100);
...
window.getFirebaseReference = getFirebaseReference;
window.getFirebaseStorageReference = getFirebaseStorageReference;
...
console.log('✅ app-core.js المحسن والمصحح loaded');